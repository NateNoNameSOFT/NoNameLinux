...
static const char *BUILD_BASE = "/usr/share/nnpkm/build/build_";
...
bool package_build(char *root, nnl_package *pck){
    ...
    sprintf(package_path, "%s/usr/share/nnpkm/build/%s-%s-nnl-1.0.0.txz", root, pck->name, pck->version);

    if(access(package_path, F_OK) == 0){
        printf("Not rebuilding, %s already built. Delete the file to rebuild\n", package_path);
        sleep(2);
        return true;
    }
    ...
    char *pck_install = malloc(strlen(pck_base) + strlen(INST_DIR) + 2);
    ...
    if(access(build_base, F_OK) == 0){
        ...
        if(mkdir(pck_base, 0775)){
            ...
            failure = true;
        } else if(!copy_dir_contents(pck->repo_path, build_base)){ //ADD FUNCTION
    ...
    if(access(build_install, F_OK) == 0){
        //.install directory exists, move it to pckdir
    ...
    while(l &&l->str){
        ...
        if(access(extra_filename, F_OK) == 0){
            char *extra_file = malloc(strlen(build_base) + strlen(basename(l->str)) + 2);
    ...
    //running perform_build as a child process (as chroot)
